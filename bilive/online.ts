...
import { liveOrigin, apiLiveOrigin, _options, _user } from './index'
/**
 * 挂机得经验
 * 
 * @export
...
export class Online extends EventEmitter {
  ...
  public OnlineHeart() {
    ...
    _user.forEach(async (User, uid) => {
      ...
      // 客户端
      let heartbeatQuery = `access_key=${User.userData.accessToken}&${AppClient.baseQuery}`
   ...
   */
  public DoLoop() {
    let eventRooms = _options.config.eventRooms
    _user.forEach(User => {
      // 每日签到
      if (User.userData.doSign) this._DoSign(User).catch((reject) => { tools.Error(User.userData.nickname, '每日签到', reject) })
      // 每日宝箱
      if (User.userData.treasureBox) this._TreasureBox(User).catch((reject) => { tools.Error(User.userData.nickname, '每日宝箱', reject) })
      // 日常活动
      if (User.userData.eventRoom && eventRooms.length > 0) this._EventRoom(User, eventRooms).catch((reject) => { tools.Error(User.userData.nickname, '日常活动', reject) })
      // 自动送礼
      if (User.userData.sendGift && User.userData.sendGiftRoom !== 0) this._sendGift(User).catch((reject) => { tools.Error(User.userData.nickname, '自动送礼', reject) })
      // 应援团签到
      if (User.userData.signGroup) this._signGroup(User).catch((reject) => { tools.Error(User.userData.nickname, '应援团签到', reject) })
    })
  }
  /**
   * 每日签到
   ...
   * @memberof Online
   */
  private async _DoSign(User: User) {
    let baseQuery = `access_key=${User.userData.accessToken}&${AppClient.baseQuery}`
      // 签到
    ...
    if (signInfo.response.statusCode === 200 && signInfo.body.code === 0) tools.Log(User.userData.nickname, '每日签到', '已签到')
    // 道具包裹
    ...
    if (getBagGift.response.statusCode === 200 && getBagGift.body.code === 0) tools.Log(User.userData.nickname, '每日签到', '已获取每日包裹')
  }
  /**
   * 每日宝箱
   * 
   ...
   * @memberof Online
   */
  private async _TreasureBox(User: User) {
    let baseQuery = `access_key=${User.userData.accessToken}&${AppClient.baseQuery}`
      // 获取宝箱状态,换房间会重新冷却
    ...
    if (currentTask.response.statusCode === 200) {
      if (currentTask.body.code === 0) {
        ...
        this._TreasureBox(User)
      }
      else if (currentTask.body.code === -10017) tools.Log(User.userData.nickname, '每日宝箱', '已领取所有宝箱')
    }
  }
  /**
   * 日常活动
   ...
   * @memberof Online
   */
  private async _EventRoom(User: User, roomIDs: number[]) {
    // 分享房间
    let biliUID = User.userData.biliUID
      , md5 = tools.Hash('md5', `${biliUID}${roomIDs[0]}bilibili`)
      , sha1 = tools.Hash('sha1', `${md5}bilibili`)
      , baseQuery = `access_key=${User.userData.accessToken}&${AppClient.baseQuery}`
    ...
    if (shareCallback.response.statusCode === 200 && shareCallback.body.code === 0) tools.Log(User.userData.nickname, '日常活动', `分享房间 ${roomIDs[0]} 成功`)
    else tools.Log(User.userData.nickname, '日常活动', shareCallback.body.msg)
    // 做任务
    ...
    task.forEach(value => {
      ...
      tools.XHR(task, 'WebView').catch(error => { tools.Error(User.userData.nickname, '日常活动', error) })
    })
  }
  /**
   * 自动送礼
   ...
   * @memberof Online
   */
  private async _sendGift(User: User) {
    let roomID = User.userData.sendGiftRoom
      , baseQuery = `access_key=${User.userData.accessToken}&${AppClient.baseQuery}`
      // 获取房间信息
        ...
        bagInfo.body.data.forEach(async giftData => {
          ...
          if (giftData.expireat > 0 && giftData.expireat < 86400) {
            ...
            if (sendBag.response.statusCode === 200 && sendBag.body.code === 0) {
              ...
              tools.Log(User.userData.nickname, '自动送礼', `向房间 ${roomID} 赠送 ${sendBagData.gift_num} 个${sendBagData.gift_name}`)
            }
            else tools.Log(User.userData.nickname, '自动送礼', `向房间 ${roomID} 赠送 ${giftData.gift_num} 个${giftData.gift_name} 失败`, sendBag.body)
          }
        })
      }
      else tools.Log(User.userData.nickname, '自动送礼', '获取包裹信息失败', bagInfo.body)
    }
    else tools.Log(User.userData.nickname, '自动送礼', '获取房间信息失败', roomInfo.body)
  }
  /**
   * 应援团签到
   * 
   ...
   * @memberof Online
   */
  private async _signGroup(User: User) {
    let baseQuery = `access_key=${User.userData.accessToken}&${AppClient.baseQuery}`
      // 获取已加入应援团列表
    ...
    if (linkGroup.response.statusCode === 200 && linkGroup.body.code === 0 && linkGroup.body.data.list.length > 0) {
      linkGroup.body.data.list.forEach(async groupInfo => {
        ...
        if (signGroup.response.statusCode === 200 && signGroup.body.data.add_num > 0) tools.Log(User.userData.nickname, '应援团签到', `在${groupInfo.group_name}签到获得 ${signGroup.body.data.add_num} 点亲密度`)
        else tools.Log(User.userData.nickname, '应援团签到', `已在${groupInfo.group_name}签到过`)
      })
    }
    else tools.Log(User.userData.nickname, '应援团签到', '获取应援团列表失败', linkGroup.body)
  }
}